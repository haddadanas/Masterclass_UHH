...
const enStepTexts: { [key in keyTypes]: { [key: string]: [string, string] } } = {
    ...
    analysis: {
        wilcome: ["Analysis Tutorial", "Now we want to find the Higgs boson. For this an automated event selection is used. Again we load an example event first."],
        selectionTools: [
            "Selection Tools",
...
const deStepTexts: { [key in keyTypes]: { [key: string]: [string, string] } } = {
    ...
    analysis: {
        welcome: ["Analysis Tutorial", "Jetzt wollen wir das Higgs Boson finden. Für diesen Zweck gibt es eine automatisierte Auswahl von Events. Wir laden wieder einen Beispiel-Event."],
        selectionTools: [
            "Auswahlwerkzeuge",
